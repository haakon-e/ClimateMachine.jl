...
function vars_state(m::KinematicModel, ::Conservative, FT)
    @vars begin
        ρ::FT
        ρu::SVector{3, FT}
        ρe::FT
...
function main()
    ...
    ρq_liq_ind = varsindex(vars_state(model, Conservative(), FT), :ρq_liq)
    ρq_ice_ind = varsindex(vars_state(model, Conservative(), FT), :ρq_ice)
    ρq_rai_ind = varsindex(vars_state(model, Conservative(), FT), :ρq_rai)
    ρq_sno_ind = varsindex(vars_state(model, Conservative(), FT), :ρq_sno)
