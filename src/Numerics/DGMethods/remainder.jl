...
function remainder_DGModel(
    ...
    for subdg in subsdg
        @assert number_states(subdg.balance_law, Conservative(), FT) <=
                number_states(maindg.balance_law, Conservative(), FT)
...
function wavespeed(
    ...
    ws = fill(
        -zero(FT),
        MVector{number_states(rem_balance_law.main, Conservative(), FT), FT},
    )
    rs = fill(
        -zero(FT),
        MVector{number_states(rem_balance_law.main, Conservative(), FT), FT},
...
function numerical_flux_first_order!(
    ...
    ntuple(Val(length(rem_balance_law.subs))) do k
        ...
        @inbounds if rem_balance_law.subsdir[k] isa Union{Dirs.types...}
            ...
            FT = eltype(a_fluxᵀn)
            num_state_conservative = number_states(sub, Conservative(), FT)
            ...
            state_rng = static(1):static(number_states(sub, Conservative(), FT))
            ...
            numerical_flux_first_order!(
                nf,
                sub,
                Vars{vars_state(sub, Conservative(), FT)}(a_sub_fluxᵀn),
                normal_vector,
                Vars{vars_state(sub, Conservative(), FT)}(
                    a_sub_state_conservative⁻,
                ),
                state_auxiliary⁻,
                Vars{vars_state(sub, Conservative(), FT)}(
