...
dofs_per_elem_reduced(N::Int, dim::Int) = # better name for this method?
    dofs_per_elem_per_direction(N) *
    dofs_per_elem_per_reduced_direction(N, dim)
...
# A NamedTuple of all `vars_state`'s.
all_vars_states(balance_law, FT) = (
    prognostic = vars_state(balance_law, Prognostic(), FT),
    auxiliary = vars_state(balance_law, Auxiliary(), FT),
    gradient = vars_state(balance_law, Gradient(), FT),
    grad_flux = vars_state(balance_law, GradientFlux(), FT),
    grad_lap = vars_state(balance_law, GradientLaplacian(), FT),
    up_integrals = vars_state(balance_law, UpwardIntegrals(), FT),
    dn_integrals = vars_state(balance_law, DownwardIntegrals(), FT),
    hyperdiff = vars_state(balance_law, Hyperdiffusive(), FT),
)
...
@kernel function volume_tendency!(
    ...
    @uniform begin
        ...
        vs = all_vars_states(balance_law, FT)
    ...
    @inbounds begin
        ...
        @unroll for k in 1:Nqk
            ...
            flux_first_order!(
                balance_law,
                Grad{vs.prognostic}(local_flux),
                Vars{vs.prognostic}(local_state_prognostic),
                Vars{vs.auxiliary}(local_state_auxiliary),
                t,
                (direction,),
            )
            ...
            flux_second_order!(
                ...
                Grad{vs.prognostic}(local_flux),
                Vars{vs.prognostic}(local_state_prognostic),
                Vars{vs.grad_flux}(local_state_gradient_flux),
                Vars{vs.hyperdiff}(local_state_hyperdiffusion),
                Vars{vs.auxiliary}(local_state_auxiliary),
                t,
            )
            ...
            source!(
                balance_law,
                Vars{vs.prognostic}(local_source),
                Vars{vs.prognostic}(local_state_prognostic),
                Vars{vs.grad_flux}(local_state_gradient_flux),
                Vars{vs.auxiliary}(local_state_auxiliary),
                t,
    ...
    @inbounds for f in faces
        ...
        if bctype == 0
            numerical_flux_first_order!(
                numerical_flux_first_order,
                balance_law,
                Vars{vs.prognostic}(local_flux),
                SVector(normal_vector),
                Vars{vs.prognostic}(local_state_prognostic⁻),
                Vars{vs.auxiliary}(local_state_auxiliary⁻),
                Vars{vs.prognostic}(local_state_prognostic⁺nondiff),
                Vars{vs.auxiliary}(local_state_auxiliary⁺nondiff),
                t,
                face_direction,
            )
            numerical_flux_second_order!(
                ...
                Vars{vs.hyperdiff}(local_state_hyperdiffusion⁻),
                Vars{vs.auxiliary}(local_state_auxiliary⁻),
                Vars{vs.prognostic}(local_state_prognostic⁺diff),
                Vars{vs.grad_flux}(local_state_gradient_flux⁺),
                Vars{vs.hyperdiff}(local_state_hyperdiffusion⁺),
                Vars{vs.auxiliary}(local_state_auxiliary⁺diff),
                t,
            )
        else
            ...
            numerical_boundary_flux_first_order!(
                ...
                t,
                face_direction,
                Vars{vs.prognostic}(local_state_prognostic_bottom1),
                Vars{vs.auxiliary}(local_state_auxiliary_bottom1),
            )
            numerical_boundary_flux_second_order!(
                numerical_flux_second_order,
                balance_law,
                ...
                t,
                Vars{vs.prognostic}(local_state_prognostic_bottom1),
                Vars{vs.grad_flux}(local_state_gradient_flux_bottom1),
                Vars{vs.auxiliary}(local_state_auxiliary_bottom1),
            )
        end
    ...
    @inbounds @views begin
        ...
        @unroll for k in 1:Nqk
            ...
            compute_gradient_argument!(
                balance_law,
                Vars{vs.gradient}(local_transform),
                Vars{vs.prognostic}(local_state_prognostic[:, k]),
                Vars{vs.auxiliary}(local_state_auxiliary[:, k]),
                t,
            )
            @unroll for s in 1:ngradstate
        ...
        @unroll for k in 1:Nqk
            ...
            if ns.grad_flux > 0
                ...
                compute_gradient_flux!(
                    balance_law,
                    Vars{vs.grad_flux}(local_state_gradient_flux),
                    Grad{vs.gradient}(local_transform_gradient[:, :, k]),
                    Vars{vs.prognostic}(local_state_prognostic[:, k]),
                    Vars{vs.auxiliary}(local_state_auxiliary[:, k]),
                    t,
                )
...
@kernel function interface_gradients!(
    ...
    @inbounds for f in faces
        ...
        if bctype == 0
            ...
            if ns.grad_flux > 0
                compute_gradient_flux!(
                    balance_law,
                    Vars{vs.grad_flux}(local_state_gradient_flux),
                    Grad{vs.gradient}(local_transform_gradient),
                    Vars{vs.prognostic}(local_state_prognostic⁻),
                    Vars{vs.auxiliary}(local_state_auxiliary⁻),
                    t,
                )
        ...
        compute_gradient_flux!(
            balance_law,
            Vars{vs.grad_flux}(local_state_prognostic⁻visc),
            Grad{vs.gradient}(l_nG⁻),
            Vars{vs.prognostic}(local_state_prognostic⁻),
            Vars{vs.auxiliary}(local_state_auxiliary⁻),
            t,
        )
...
@kernel function kernel_init_state_prognostic!(
    ...
    Nq = dofs_per_elem_per_direction(N)
    Nqk = dofs_per_elem_per_reduced_direction(N, dim)
    Np = dofs_per_elem_reduced(N, dim)
        ...
        for ev in 1:nvertelem
            ...
            @unroll for k in 1:Nq
                ...
                integral_load_auxiliary_state!(
                    balance_law,
                    Vars{vs.up_integrals}(view(local_kernel, :, k)),
                    Vars{vs.prognostic}(local_state_prognostic),
                    Vars{vs.auxiliary}(local_state_auxiliary),
                )
            ...
            @unroll for k in 1:Nq
                ...
                integral_set_auxiliary_state!(
                    balance_law,
                    Vars{vs.auxiliary}(view(state_auxiliary, ijk, :, e)),
                    Vars{vs.up_integrals}(view(local_kernel, :, k)),
                )
...
@kernel function kernel_reverse_indefinite_stack_integral!(
    ...
    @inbounds begin
        ...
        reverse_integral_load_auxiliary_state!(
            balance_law,
            Vars{vs.dn_integrals}(l_T),
            Vars{vs.prognostic}(view(state, ijk, :, et)),
            Vars{vs.auxiliary}(view(state_auxiliary, ijk, :, et)),
        )
        ...
        for ev in 1:nvertelem
            ...
            @unroll for k in 1:Nq
                ...
                reverse_integral_load_auxiliary_state!(
                    balance_law,
                    Vars{vs.dn_integrals}(l_V),
                    Vars{vs.prognostic}(view(state, ijk, :, e)),
                    Vars{vs.auxiliary}(view(state_auxiliary, ijk, :, e)),
                )
                l_V .= l_T .- l_V
    ...
    @inbounds for f in faces
        ...
        if bctype == 0
            numerical_flux_divergence!(
                divgradnumpenalty,
                balance_law,
                Vars{vs.grad_lap}(l_div),
                normal_vector,
                Grad{vs.grad_lap}(l_grad⁻),
                Grad{vs.grad_lap}(l_grad⁺),
            )
        else
            numerical_boundary_flux_divergence!(
