...
function DGModel(
    ...
    fill_nan = false,
    state_auxiliary = create_state(balance_law, grid, Auxiliary(), fill_nan = fill_nan),
...
function init_ode_state(dg::DGModel, args...; init_on_cpu = false, fill_nan = false)
    device = arraytype(dg.grid) <: Array ? CPU() : CUDADevice()

    balance_law = dg.balance_law
    grid = dg.grid

    state_prognostic = create_state(balance_law, grid, Prognostic(), fill_nan = fill_nan)
