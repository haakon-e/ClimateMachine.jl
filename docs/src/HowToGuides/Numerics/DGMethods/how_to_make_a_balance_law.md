...
```
∂Y
-- = - ∇ • ( F_{first_order}(Y) + F_{second_order}(Y, Σ) ) + S_{non_prognostic}(Y, Σ)
∂t
```

Here, `Y`, `Σ`, `F_{first_order}`, `F_{second_order}`, and
`S_{non_prognostic}` can be thought of column vectors[^1] expressing:
 ...
 - `S_{non_prognostic}` non-conservative sources[^2]
...
|:-----|:---|:----|
| [`vars_state`](@ref)         | **YES** |  specify the names of the variables in the prognostic state vector, typically mass, momentum, and various tracers. |
...
| [`vars_state`](@ref)         | **YES** |  specify the names of the gradients of functions of the prognostic state variables. used to represent values before **and** after differentiation |
| [`vars_state`](@ref)         | **YES** |  specify the names of the gradient fluxes necessary to impose Neumann boundary conditions. typically the product of a diffusivity tensor with a gradient state variable, potentially equivalent to the second-order flux for a prognostic state variable |
...
| [`source!`](@ref)            |  specify `S_{non_prognostic}`  for each prognostic state variable. can be functions of the prognostic state, gradient flux state, and auxiliary variables. |
